...
| S3 | Causal conservation test: does M(V) recover ground-truth causal graph? | PCMCI; runs 1–2 (S1) → S3-bis (S1-bis, 48-cycle) | ❌ Property 1 REFUTED (§6.8) | S3-bis: raw F1=1.000 (clean ceiling), reconstruction F1=0.135 → low-rank Tucker destroys causal structure (semantic collapse) |
...
PCMCI fixed run-1's false-positive problem (raw precision 1.00, zero transitive/reverse edges) but recovered only the first edge of each chain (3→5, 7→6, 4→8), missing the second (5→6, 6→1, 8→9). A post-hoc diagnostic (reported, not substituted for run 2) shows PCMCI recovers *both* edges of every chain when sessions are concatenated (360 time points, 4 total predictions) — so the bottleneck is neither the algorithm nor a corpus defect, but the **pre-registered unit of analysis**: an independent per-session estimate plus majority vote demands more temporal evidence than an attenuated second link supplies in 12 cycles. Run 2 thus correctly answers the pre-registered question, which proved more restrictive than understood when written. Raw F1=0.667 sits 0.033 below the 0.70 ceiling; by experimental discipline the classification stays **inconclusive**, though the evidence (0.667 ≈ 0.70) is qualitatively distinct from a method that fails outright. Per the amended rule we stop and report rather than iterate methods. (An external methodological review concurred: the stop is correct, run 2 is not invalidated, and Granger→PCMCI is controlled debugging, not method-shopping. Reusing concatenation now would change the unit of analysis post-hoc — off-limits; a pooled unit of analysis is legitimate only if fixed in a new pre-registration.)
...
Causal-F1 rises **monotonically with κ(V)**: the compression that achieved 98% variance (§6.6) trades away causal structure. This measures the semantic-collapse mechanism (§3) as a curve rather than asserting it. We claim the **direction** as robust (more compression → less causal structure) and treat the **absolute levels** as provisional: because the raw control is capped at 0.667, the reconstruction levels are confounded and would re-scale if a future corpus lifts the ceiling, so they are not read as "C preserves X% of causality." Per the external review, this trade-off — not the PASS/FAIL of Property 1 — is the strongest result of S3 runs 1–2. **Decision (per pre-registration):** stop method iteration; the bottleneck is the pre-registered per-session-plus-vote unit of analysis, not the algorithm or a corpus defect. A clean Property-1 gate requires a NEW pre-registration with the unit of analysis fixed in advance — pursued in §6.8 (S1-bis). Runs 1–2 documented in `causal_conservation/NEGATIVE_RESULTS.md`.

### 6.8 S1-bis + S3-bis: Clean Gate — Property 1 REFUTED

Path A from §6.7 was executed under a joint pre-registration fixed *before the corpus existed* (the causal gate written first makes tuning the corpus toward it impossible). S1-bis regenerates the corpus with the single change `t_cycles: 12 → 48`; the two length-sensitive parameters (`recover_rate`, `drift_std`) are scaled by the a-priori rule `÷(t_cycles/12)` so accumulated drift and shock-recovery fraction match S1; everything else (weights, lag, shock, noise, 3 graphs, n=30) is identical. The S1-bis self-test (a deliberately weaker *bivariate* abort gate, not the causal gate) passed at separation +0.574. S3-bis then ran the **byte-identical** PCMCI method and per-session+vote unit of analysis (imported from run 2) on the longer corpus.
...
**Stage 1 — precondition MET (raw F1 = 1.000).** With 48 cycles and the unchanged unit of analysis, PCMCI recovers all 6 ground-truth edges with zero false positives. This quantitatively confirms the diagnosis: the run-2 ceiling (0.667) was a shortage of per-session temporal degrees of freedom for the partial-correlation test on the second chain edge — not the method, not C. The unit of analysis is no longer the bottleneck.

**Stage 2 — Property 1 REFUTED (reconstruction F1 = 0.135).** Against a perfect raw control, the low-rank Tucker reconstruction invents 77 spurious causal edges (precision 0.072). Because the control is perfect, this collapse is unambiguously attributable to **C, not the method** — a clean refutation, not an inconclusive. With the raw ceiling now valid, the κ(V)–F1 trade-off levels become interpretable: even at κ=1296 (minimal compression tested) F1 reaches only 0.245. **No tested compression regime preserves causality.**

**Implication.** Property 1 (causal preservation) is **refuted for low-rank Tucker as the sole composition operator C**. C satisfies Property 3 (dimensional stability) and Property 4 (tractability), and achieves 98% variance fidelity — but it does *not* preserve the causal structure between quality dimensions. The result is sharp, not fatal: **variance-optimal compression and causality-preserving compression are different objectives**, and a governance composition operator must target the latter. This motivates causality-aware variants of C (a causal-support-constrained core, or an explicit cycle-mode-rank/causal-fidelity trade-off) as separately pre-registered future work. The pre-registration discipline is what makes this refutation attributable to C: the two-stage gate was fixed before the corpus existed. Documented in `causal_conservation/NEGATIVE_RESULTS.md`.
