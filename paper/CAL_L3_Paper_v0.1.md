...
| S3 | Causal conservation test: does M(V) recover ground-truth causal graph? | Granger (run 1) → PCMCI (run 2) | ⚠️ Inconclusive ×2 (§6.7) | Raw F1=0.667 (0.033 below ceiling); bottleneck = per-session unit of analysis; κ(V)-F1 trade-off is the key result; reviewer-validated |
...
Causal-F1 rises **monotonically with κ(V)**: the compression that achieved 98% variance (§6.6) trades away causal structure. This measures the semantic-collapse mechanism (§3) as a curve rather than asserting it. We claim the **direction** as robust (more compression → less causal structure) and treat the **absolute levels** as provisional: because the raw control is capped at 0.667, the reconstruction levels are confounded and would re-scale if a future corpus lifts the ceiling, so they are not read as "C preserves X% of causality." Per the external review, this trade-off — not the PASS/FAIL of Property 1 — is the strongest result of S3. **Decision (per pre-registration):** stop method iteration; the bottleneck is the pre-registered per-session-plus-vote unit of analysis, not the algorithm or a corpus defect. A clean Property-1 gate requires a NEW pre-registration with the unit of analysis fixed in advance — either S1-bis (longer sessions, t_cycles≈40–60, same per-session unit) to lift the raw ceiling ≥ 0.70, or a pooled unit of analysis fixed up front (reusing concatenation post-hoc is off-limits). S2 and S4 are unaffected; Property 1 remains **open**, with qualitative evidence that low-rank Tucker erodes causal structure. Runs 1–2 documented in `causal_conservation/NEGATIVE_RESULTS.md`.
